...
利用 MiMo Omni 系列 (`mimo-v2.5` / `mimo-v2-omni`) 的多模态能力，扩展有声书体验。

### 图片理解 (mimo-v2.5)

- **图文小说支持** — 识别小说中的插图/漫画，自动生成旁白描述融入朗读
- **封面/插图描述** — 为视障用户描述书籍封面和章节插图
- **输入方式**: URL / Base64，单张 ≤50MB
- **格式**: JPEG, PNG, GIF, WebP, BMP

### 音频理解 (mimo-v2.5)

- **参考音频分析** — 声音克隆时分析用户上传的样本质量
- **有声书质检** — 生成后自动检测音频质量（断句、清晰度）
- **输入方式**: URL / Base64，单个 ≤100MB (URL) / ≤50MB (Base64)
